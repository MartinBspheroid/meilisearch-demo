...
async function loadTransactionData(): Promise<void> {
  try {
    ...
    if (await jsonFile.exists()) {
      ...
      console.warn("⚠️ transactions.json not found - run 'bun generate_financial_data.ts' first");
    }
  } catch (error) {
  ...
  try {
    ...
    if (await csvFile.exists()) {
      ...
      console.warn("⚠️ transactions.csv not found - run 'bun generate_financial_data.ts' first");
    }
  } catch (error) {
    console.error("❌ Error loading transactions.csv:", error);
  }
}
